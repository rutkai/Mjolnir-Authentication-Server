...
function sessionHasJoined(request, response) {
    ...
    if (!request.query.username) {
        ...
        response.json({
            "error": "IllegalArgumentException",
            "errorMessage": "Username can not be null or empty."
        });
        return;
    }

    if (!request.query.serverId) {
        logger.log("  Missing serverId");
        response.json({
            "error": "IllegalArgumentException",
    ...
    if (!userManager.sessionHasJoined(request.query.username, request.query.serverId)) {
        // Resposne test
        return;
    }
